...
class PointIntersection:
  ...
  def findSequence(self):
        ...
        if indexSharp in [0,1,2,3,4]:
            return dictionary
        # рассмотрим строку и поищем в ней строки с условием и циклы
        # и если они отсутствуют - значит обычная строка, в которой ищем операторы
        haveIf=False
        haveCycle=False
        template_if="\s*(if)\s+"
        lst=re.findall(template_if,self._code)
        if len(lst)!=0:
            haveIf=True
        if haveIf==False:
            patternCycles=["\s*(for)\s*","\s*(end\s+while)\s+","\s*(while)\s*",]
            for template in patternCycles:
                lst=re.findall(template,self._code)
                if len(lst)!=0:
                    haveCycle=True
                    break
        # поиск в строке разделителей условий
        if haveIf:
            ...
            self._after=lst_ops
            return dictionary
        # поиск в строке разделителей в условии цикла
        if haveCycle:
            # ищем с помощью регулярных выражений логические операторы
            lst=re.findall(patternCycle,self._code)
            for i in lst:
                if len(i)!=0:
                    for j in i:
...
if __name__ == '__main__':
    #string="+= ** + .eql? = && "
    string="end while k<1000 && d>3435 || a<0"
    print(PointIntersection(string,0,0,"=").findSequence)
    string="end while k<1000 and d>3435 or a<0"
    print(PointIntersection(string,0,0,"=").findSequence)
    string="end while k<1000 not d>3435 || a<0"
    print(PointIntersection(string,0,0,"=").findSequence)
    string="for i in [true, true, false].map{ |p| c || p };"
    print(PointIntersection(string,0,0,"=").findSequence)
